...
if [[ ! -d "$bridge_root/node_modules/node-hid" || ! -f "$bridge_root/dist/daemon.js" ]]; then
  tmux display-message "claude-micro: dependencies or build missing — run: pnpm install --dir $root"
  exit 0
fi
