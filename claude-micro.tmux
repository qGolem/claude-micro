...
if [[ ! -d "$bridge_root/node_modules/node-hid" ]]; then
  tmux display-message "claude-micro: dependencies missing — run: pnpm install --dir $root"
  exit 0
fi
...
status_command="$node_bin $bridge_root/src/tmux-status.mjs"
...
if [[ "$slot_bindings" != "off" ]]; then
  for slot in 0 1 2 3 4 5; do
    tmux bind-key -n -N "Focus Claude Micro slot $((slot + 1))" "M-$((slot + 1))" run-shell "$node_bin $bridge_root/src/focus-slot.mjs $slot"
  done
fi
